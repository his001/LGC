...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: LGC.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: LGCd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeLGCamount=<n> " + _("Keep N LGC anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. LGC is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. LGC is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("LGC version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizeLGCAmount = GetArg("-anonymizeLGCamount", 0);
    if(nAnonymizeLGCAmount > 999999) nAnonymizeLGCAmount = 999999;
    if(nAnonymizeLGCAmount < 2) nAnonymizeLGCAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeLGCAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    /* Denominations
       ...
       For example:
       1LGC+1000 == (.1LGC+100)*10
       10LGC+10000 == (1LGC+1000)*10
    */
