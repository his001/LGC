...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #LGCTEST\r");
            Send(hSocket, "WHO #LGCTEST\r");
        } else
        {
	    LogPrintf("JOIN #LGC\n");
            // randomly join #LGC00-#LGC05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #LGC%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #LGC%02d\r", channel_number).c_str());
        };
