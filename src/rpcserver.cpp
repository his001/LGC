    ...
    while (fRun)
    {
        ...
        try
        {
            ...
            if (valRequest.type() == obj_type) {
                ...
                LogPrintf("rpcserver.cpp strReply [787] %s\n",strReply); //PHS
            // array of requests
...
json_spirit::Value CRPCTable::execute(const std::string &strMethod, const json_spirit::Array &params) const
{
    try
    {
        ...
        {
            // Find method
            LogPrintf("rpcserver.cpp CRPCTable::execute [818] \n"); //PHS
            const CRPCCommand *pcmd = tableRPC[strMethod];

            if (!pcmd){
                LogPrintf("rpcserver.cpp CRPCTable::execute !pcmd [822] \n"); //PHS
            ...
            if (pcmd->reqWallet && !pwalletMain)
            {
                LogPrintf("rpcserver.cpp CRPCTable::execute pcmd->reqWallet && !pwalletMain [828] \n"); //PHS
            ...
            {
                LogPrintf("rpcserver.cpp CRPCTable::execute strWarning [837] \n"); //PHS
                throw JSONRPCError(RPC_FORBIDDEN_BY_SAFE_MODE, string("Safe mode: ") + strWarning);
            }

            if (pcmd->threadSafe)
            {
                LogPrintf("rpcserver.cpp CRPCTable::execute pcmd->threadSafe [843]\n" ); //PHS,params.string()
                //try{
//result = pcmd->actor(params, false); //PHS 변경
                //}catch (std::exception& e){
                    //LogPrintf("rpcserver.cpp [847] \n"); //PHS
                //  result = NULL;
                //}
result = NULL;
            }
                ...
                LogPrintf("rpcserver.cpp CRPCTable::execute !pwalletMain [855] \n" ); //PHS
                LOCK(cs_main);
                result = pcmd->actor(params, false);
            } else {
                LogPrintf("rpcserver.cpp CRPCTable::execute _else_ [859] \n"); //PHS
                ...
                LOCK(cs_main);
                LogPrintf("rpcserver.cpp CRPCTable::execute ENABLE_WALLET [866] \n"); //PHS
                result = pcmd->actor(params, false);
            }
#endif // !ENABLE_WALLET
        }
        LogPrintf("rpcserver.cpp CRPCTable::execute result [871] \n"); //PHS
        return result;
    }
    catch (std::exception& e)
    {
        LogPrintf("rpcserver.cpp CRPCTable::execute [876] \n"); //PHS
