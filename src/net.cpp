...
CAddress addrSeenByPeer(CService("127.0.0.1", 35351), nLocalServices);
...
CAddress GetLocalAddress(const CNetAddr *paddrPeer)
{
  ...
  CAddress ret(CService("127.0.0.1",35351),0);
    CService addr;
...
void CNode::PushVersion()
{
    ...
    CAddress addrYou = (addr.IsRoutable() && !IsProxy(addr) ? addr : CAddress(CService("127.0.0.1",35351)));
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. LGC is probably already running."), addrBind.ToString().c_str());
        else
